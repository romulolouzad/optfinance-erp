...
export default function FormRegistrarPagamento({ parcela, onClose, onConfirm }) {
  const hoje = new Date().toISOString().slice(0, 10)

  const [valorRecebido, setValorRecebido] = useState(
    parcela ? (parcela.valorBruto - parcela.valorRecebido) : 0
  )
  const [dataRecebimento, setDataRecebimento] = useState(hoje)
  const [contaId, setContaId] = useState(parcela?.contaFinanceiraId || 'CF001')
  ...
  const saldoRestante = parcela ? parcela.valorBruto - parcela.valorRecebido - valorRecebido : 0
  ...
  function validate() {
    ...
    if (pagamentoParcial && !novoVencimento) errs.novoVencimento = 'Informe o novo vencimento do saldo restante.'
    if (pagamentoParcial && valorRecebido >= parcela.valorBruto - parcela.valorRecebido) {
  ...
  const valorEmAberto = parcela.valorBruto - parcela.valorRecebido
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-lg bg-surface-container-lowest rounded-2xl shadow-2xl flex flex-col overflow-hidden"
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-surface-container">
          <div>
            <h2 className="text-base font-bold text-on-surface tracking-tight">Registrar Pagamento</h2>
            <p className="text-xs text-text-muted mt-0.5">
              Parcela {parcela.numero} — {parcela.clienteNome}
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-xl bg-surface-container p-3">
              <p className="text-[10px] text-text-muted uppercase tracking-widest font-semibold mb-1">Valor da Parcela</p>
              <p className="text-sm font-bold text-on-surface">{fmt(parcela.valorBruto)}</p>
            </div>
            <div className="rounded-xl bg-surface-container p-3">
              <p className="text-[10px] text-text-muted uppercase tracking-widest font-semibold mb-1">Em Aberto</p>
              <p className="text-sm font-bold text-on-surface">{fmt(valorEmAberto)}</p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-text-muted text-sm font-semibold">R$</span>
              <input
                type="number"
                min="0.01"
                step="0.01"
            ...
            {pagamentoParcial && valorRecebido > 0 && saldoRestante > 0 && (
              <p className="text-xs text-text-muted mt-1.5">
                Saldo restante após este pagamento: <span className="font-semibold text-on-surface">{fmt(saldoRestante)}</span>
              </p>
            )}
          </div>
          ...
          <div className="rounded-xl bg-surface-container p-4">
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() => { setPagamentoParcial(p => !p); setErrors(e => ({ ...e, novoVencimento: undefined })) }}
            ...
            {pagamentoParcial && (
              <div className="mt-4 pt-4 border-t border-surface-container-high">
                ...
                />
                {errors.novoVencimento && <p className="text-xs text-error mt-1">{errors.novoVencimento}</p>}
              </div>
            )}
          </div>
        </div>
